...
export default function PreviewModal({ isOpen, onClose, title, dataType, data }: PreviewModalProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // 1. Create a hidden sandbox container off-screen
      const container = document.createElement("div");
      container.id = "pdf-sandbox-container";
      container.style.position = "absolute";
      container.style.left = "-9999px";
      container.style.top = "-9999px";
      container.style.width = "720px"; // Stable print friendly width
      container.style.backgroundColor = "#ffffff";
      container.style.color = "#1e293b";
      container.style.padding = "35px";
      container.style.boxSizing = "border-box";
      
      // Inject theme-royal-ivory class so all inner elements render in light mode perfectly!
      container.className = "theme-royal-ivory";

      // 2. Inject an official document header at the very top of the PDF page
      const bday = new Date();
      const documentHeader = document.createElement("div");
      documentHeader.innerHTML = `
        <div style="font-family: 'Hind Siliguri', 'Inter', sans-serif; text-align: center; border-bottom: 3px double #4f46e5; padding-bottom: 18px; margin-bottom: 25px;">
          <h1 style="font-size: 26px; color: #4f46e5; margin: 0; font-weight: 800; letter-spacing: 0.5px;">অক্ষর পাঠাগার (Akkhor Pathagar)</h1>
          <p style="font-size: 12px; color: #475569; margin: 6px 0 0 0; font-weight: 500;">একটি আধুনিক ও সুরক্ষিত লাইব্রেরি ম্যানেজমেন্ট সিস্টেম</p>
          <div style="display: flex; justify-content: space-between; margin-top: 15px; font-size: 10px; color: #64748b; border-top: 1px solid #f1f5f9; padding-top: 8px;">
            <span>তারিখ: ${bday.toLocaleDateString("bn-BD")} (বাংলাদেশ সময়)</span>
            <span style="font-family: monospace; font-weight: bold;">ডকুমেন্ট আইডি: AP-DOC-${Date.now().toString().slice(-6)}</span>
          </div>
        </div>
      `;
      container.appendChild(documentHeader);

      // 3. Clone the print content into the sandbox we created
      ...
      // 5. Instantly clean up the sandbox elements from document body
      ...
      // 7. Add an elegant professional watermark, page number tracker and branding header on each page
      const totalPages = pdf.getNumberOfPages();
      for (let i = 1; i <= totalPages; i++) {
        pdf.setPage(i);
        pdf.setFontSize(8);
        pdf.setTextColor(148, 163, 184); // Slate 400 Color
        
        // Header Watermark text
        pdf.text("OFFICIAL SYSTEM DOCUMENT • AKKHOR PATHAGAR", 15, 9);
        
        // Bottom Footer details
        pdf.text(`ডকুমেন্ট পৃষ্ঠা: ${i} / ${totalPages}`, 175, 291);
        pdf.text("© অক্ষর পাঠাগার - সর্বস্বত্ব সংরক্ষিত। হেল্পলাইন: 01333474848", 15, 291);
      }

      // Save PDF with sanitized visual filename
      const sanitizedTitle = title.replace(/[^a-zA-Z0-9\u0980-\u09FF]/g, "_");
      pdf.save(`অক্ষর_পাঠাগার_${sanitizedTitle}.pdf`);
    } catch (err) {
