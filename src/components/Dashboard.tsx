  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <span className="text-[#52525B] font-semibold font-sans">ড্যাশবোর্ড লোড হচ্ছে...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[#E5E5EA] pb-4">
        <div>
          ...
          <p className="text-[#52525B] text-xs mt-1">পাঠাগার পরিচালনার রিয়েল-টাইম পরিমাপক পরিসংখ্যান</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          >
            <Send size={14} className="text-[#D97706]" />
            অবিলম্বে SMS শিডিউল সিঙ্ক
          </button>
          <button
            onClick={onRefresh}
            className="flex items-center justify-center p-2 rounded-full bg-white border border-[#E5E5EA] hover:border-[#22242A]/30 cursor-pointer text-[#52525B] hover:text-[#22242A] shadow-[0_2px_8px_rgba(0,0,0,0.04)] transition-all"
            title="রিফ্রেশ করুন"
          >
            <RefreshCw size={16} />
          </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        >
          <div className="flex justify-between items-start">
            <span className="text-[#52525B] text-xs font-medium">মোট বই সংখ্যা</span>
        ...
        >
          <div className="flex justify-between items-start">
            <span className="text-[#52525B] text-xs font-medium">উপলব্ধ বই</span>
        ...
        >
          <div className="flex justify-between items-start">
            <span className="text-[#52525B] text-xs font-medium">ধারকৃত বই</span>
        ...
        >
          <div className="flex justify-between items-start">
            <span className="text-[#52525B] text-xs font-medium">বিলম্বিত বই (Overdue)</span>
        ...
        >
          <div className="flex justify-between items-start">
            <span className="text-[#52525B] text-xs font-medium">আজকের লেনদেন</span>
        ...
        >
          <div className="flex justify-between items-start">
            <span className="text-[#52525B] text-xs font-medium">মোট সদস্য সংখ্যা</span>
        ...
        <div className="glass-panel p-6 flex flex-col">
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-[#52525B] text-[11px]">বিগত ৬ মাসের বই ইস্যু এবং জমা রেকর্ডের তুলনামূলক রেখাচিত্র</p>
            </div>
            <div className="flex items-center gap-2 text-[10px] text-[#52525B] font-sans font-bold">
              <span className="inline-block w-2.5 h-2.5 rounded-sm bg-[#22242A]"></span>ইস্যু
              <span className="inline-block w-2.5 h-2.5 rounded-sm bg-[#D97706] ml-1"></span>ফিরতি
            </div>
          </div>
                    ...
                    {[0, 0.25, 0.5, 0.75, 1].map((ratio, i) => {
                      ...
                      return (
                        <g key={i} className="opacity-70">
                          <line x1="40" y1={yVal} x2="480" y2={yVal} stroke="#D4D4D8" strokeDasharray="3,3" strokeWidth="1" />
                          <text x="32" y={yVal + 3} fill="#52525B" fontSize="9" textAnchor="end">{num}</text>
                        </g>
                      );
                    })}
                    ...
                    {charts.monthlyReport.map((item, idx) => {
                      ...
                      return (
                        <g key={idx} className="group">
                          ...
                          {/* Label bottom */}
                          <text x={xBase} y={chartHeight - 8} fill="#3F3F46" fontSize="10" textAnchor="middle" fontWeight="600">
                            {item.month}
                          </text>
                        </g>
                      );
      ...
      <div className="glass-panel p-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-[#52525B] text-[10px] mt-1">নিচের সদস্যদের বই জমা দেওয়ার সময়সীমা অতিবাহিত হয়েছে। তাদের SMS রিকল শিডিউল সচল আছে।</p>
          </div>
          <button
            onClick={() => onNavigate("sms")}
        ...
        {charts.lateReportLoans.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-[#22242A]">
              <thead>
                <tr className="border-b border-[#E5E5EA] text-[#52525B] text-[10px] uppercase font-bold">
                ...
                {charts.lateReportLoans.map(item => (
                  <tr key={item.id} className="hover:bg-[#F5F3EF] duration-150">
                    <td className="py-3">
                      ...
                      <p className="text-[10px] text-[#52525B] font-mono mt-0.5">{item.bookCode}</p>
                    </td>
                    <td className="py-3">
                      <p className="font-bold text-[#22242A]">{item.memberName}</p>
                      <p className="text-[10px] text-[#52525B] font-mono mt-0.5">ফরম: #{item.formNumber}</p>
                    </td>
                    <td className="py-3 font-mono text-[#22242A]">{item.mobile}</td>
                    <td className="py-3 text-[#DC2626] font-bold font-mono">{item.returnDate}</td>
                    <td className="py-3">
                      <span className="px-2.5 py-1 rounded text-[9px] font-bold bg-[#DC2626]/10 text-[#DC2626]">
                        OVERDUE
                      </span>
                    </td>
                  </tr>
